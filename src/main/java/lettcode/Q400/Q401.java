...
public class Q401 {
    ...
    private void findTime(int num, int hour, int minute,int index) {
        if (num == 0) {
            String minStr = (minute < 10) ? ("0" + minute) : minute + "";
            res.add(hour + ":" + minStr);
            return;
        }
        for (int i = index; i < minuteUsed.length; i++) {
            if (!minuteUsed[i]) {
                minuteUsed[i] = true;
                findTime(num - 1, hour, minute + minutes[i],i+1);
                minuteUsed[i] = false;
            }
        }
        for (int i = index; i < hourUsed.length; i++) {
            if (!hourUsed[i]) {
                hourUsed[i] = true;
                findTime(num - 1, hour + hours[i], minute,i+1);
                hourUsed[i] = false;
            }
        }
    }
    ...
    //[0:03, 0:05, 0:09, 0:17, 0:33, 2:01, 4:01, 8:01, 0:06, 0:10, 0:18, 0:34, 4:02, 8:02, 0:12, 0:20, 0:36, 8:04,
    // 0:24, 0:40, 0:48, 1:02, 1:04, 1:08, 1:16, 1:32, 3:00, 5:00, 9:00, 2:04, 2:08, 2:16, 2:32, 6:00, 10:00, 4:08,
    // 4:16, 4:32, 12:00, 8:16, 8:32]
